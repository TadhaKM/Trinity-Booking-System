...
export default function HomePage() {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <section className="relative min-h-[85vh] flex items-center overflow-hidden" data-testid="hero-section">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center py-12 lg:py-0">
            ...
            {/* Left Column - Text */}
            <div className={`space-y-8 transition-all duration-1000 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              {user ? (
                <>
                  <div>
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <Link href="/login" data-testid="hero-login-btn" className="btn-pill bg-[#0A2E6E] text-white shadow-lg shadow-[#0A2E6E]/25 hover:shadow-xl hover:shadow-[#0A2E6E]/30 flex items-center gap-2">
                      Get Started
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                    </Link>
                    <Link href="/signup" data-testid="hero-signup-btn" className="btn-pill bg-white text-[#0A2E6E] border-2 border-[#0A2E6E]/10 hover:border-[#1A6FEF]/30 shadow-md">
                      Create Account
                    </Link>
                  </div>
                </>
            ...
            <div className={`relative hidden lg:block transition-all duration-1000 ease-out ${mounted ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`} style={{ transitionDelay: '300ms' }}>
              <div className="relative">
                {/* Main image */}
      ...
      {/* UPCOMING EVENTS                                 */}
      {/* ═══════════════════════════════════════════════ */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24" data-testid="events-section">
        {/* Section header */}
        <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-end mb-12 gap-4 transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: '200ms' }}>
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-[#0A2E6E]" data-testid="events-heading">Upcoming Events</h2>
            <p className="text-slate-500 mt-2 text-lg">Don&apos;t miss out on what&apos;s happening</p>
          </div>
          <Link href="/search" data-testid="view-all-events-link" className="group flex items-center gap-2 text-[#1A6FEF] hover:text-[#0A2E6E] font-semibold text-sm transition-colors duration-300 bg-[#1A6FEF]/5 hover:bg-[#1A6FEF]/10 px-5 py-2.5 rounded-full">
            View All
        ...
        {events.length > 0 ? (
            ...
            {events.slice(0, 6).map((event, index) => {
              ...
              return (
                <div
                  ...
                  className={`group cursor-pointer card-shine transition-all duration-700 ease-out ${
                    mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                  } ${
                    isFeatured ? 'lg:col-span-8 lg:row-span-2' : isTall ? 'lg:col-span-4 lg:row-span-2' : 'lg:col-span-4'
                  }`}
          ...
          <div className={`text-center py-20 bg-white rounded-[2rem] shadow-sm transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: '400ms' }} data-testid="no-events">
            <div className="w-20 h-20 bg-[#1A6FEF]/10 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="w-10 h-10 text-[#1A6FEF]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
            </div>
      ...
      {/* CATEGORIES                                      */}
      {/* ═══════════════════════════════════════════════ */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 md:pb-24" data-testid="categories-section">
        <div className={`mb-10 transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: '500ms' }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5" data-testid="categories-grid">
          {CATEGORIES.map((category, i) => (
            <Link
              ...
              className={`category-card block transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
            ...
            >
              ...
              >
                ...
                <span className="text-4xl block mb-3 transition-transform duration-500 group-hover:scale-125 group-hover:-rotate-12 relative z-10">{category.icon}</span>
          ...
          <div className="relative rounded-[2rem] overflow-hidden bg-[#0A2E6E] p-12 md:p-16 text-center noise-overlay">
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <Link href="/signup" data-testid="cta-signup-btn" className="btn-pill bg-white text-[#0A2E6E] font-bold shadow-lg hover:shadow-xl">
                  Create Free Account
                </Link>
                <Link href="/search" data-testid="cta-browse-btn" className="btn-pill bg-white/15 text-white border border-white/25 backdrop-blur-sm">
                  Browse Events
                </Link>
              </div>
            </div>
